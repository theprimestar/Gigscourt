...
const Navigation = (function() {
    ...
    // Add this after the variable declarations in navigation.js
...
// Modify the init() function - replace with this:
function init() {
    // Check auth first
    (async () => {
        const user = await window.SupabaseAPI?.getCurrentUser();
...
// Modify navigateTo to add auth check for protected routes
const originalNavigateTo = navigateTo;
window.Navigation.navigateTo = async function(page, params, addToStack) {
    // Protect all routes except auth (but auth doesn't need protection)
    const user = await window.SupabaseAPI?.getCurrentUser();
    if (!user && page !== "auth") {
        if (window.AuthFlow) {
            window.AuthFlow.render();
    ...
    return originalNavigateTo(page, params, addToStack);
};
    
    // Root tabs (these should reset/replace stack, not push)
    ...
    function navigateTo(page, params = {}, addToStack = true) {
        ...
        if (isRootTab) {
            ...
            // But preserve its internal stack
    ...
    function init() {
        // Initialize stacks with root pages
        reset();
        ...
        // Handle browser back button - maps to our goBack
        window.addEventListener("popstate", (e) => {
            goBack();
        });
        ...
        console.log("Navigation initialized with per-tab stacks");
    }
