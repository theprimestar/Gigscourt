...
// Add after updateGeohash function
async function updateVisibilityStatus(userId) {
    const client = await getSupabaseClient();
    const profile = await getProfile(userId);
    
    const hasServices = profile.selected_services && profile.selected_services.length > 0;
    const hasLocation = profile.latitude && profile.longitude;
    const hasPhoto = profile.avatar_url && profile.avatar_url !== "";
...
async function getVisibilityStatus(userId) {
    const client = await getSupabaseClient();
    const { data, error } = await client
        .from('profiles')
        .select('is_visible, selected_services, latitude, longitude, avatar_url')
        .eq('id', userId)
        .single();
    ...
    return {
        ...
        missing: {
            services: !data.selected_services || data.selected_services.length === 0,
...
// Add to window.SupabaseAPI export at the bottom
window.SupabaseAPI = {
    // ... existing exports
